...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
  	val z=w.toLowerCase groupBy (t=>t)
  	z.map(t => (t._1,t._2.length)).toList.sorted
	}          
  ...
  def str(w: Occurrences): String = {
  	val z =for {
  		(x1,y1) <- w
  	} yield x1.toString*y1
  	z.mkString
  	}
  
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val z = str(occurrences)
    for {
 	 x <- 0 to z.length
 	 y <- z.combinations(x)
 	 } yield wordOccurrences(y).toSeq.toList 
  }.toList
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = wordOccurrences( str(x) diff str(y) )
   ...
   */
   def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
     ...
     if (occ.isEmpty) List(List())
    	...
    	for {
    	x <- c
    	w <- dictionaryByOccurrences get x match {
    	case Some(list) => list
    	case None => List()
    	}
    	s1 <- helper(subtract(occ,x))
      } yield w::s1
      }
      }
